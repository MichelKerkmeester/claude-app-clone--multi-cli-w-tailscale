...
export function useAskQuestionState(
  viewModel: AskQuestionViewModel | null,
  options: AskQuestionStateOptions = {},
): AskQuestionStateApi {
  const identity = viewModel === null ? null : askQuestionKey(viewModel.questionId, viewModel.revision);
  const [state, dispatch] = useReducer(
    (current: AskQuestionFormState, action: AskQuestionAction) =>
      askQuestionStateReducer(current, action, viewModel),
    createAskQuestionState(viewModel, options.initialState),
  );
  ...
  useEffect(() => {
    ...
    dispatch(
      options.initialState === undefined
        ? { type: 'reset', viewModel }
        : { type: 'reset', viewModel, initialState: options.initialState },
    );
  }, [identity, options.initialState, viewModel]);

  const validation = validateAskQuestionAnswer(
    viewModel,
  ...
  const canSubmit =
    validation.valid &&
    state.phase !== 'submitting' &&
    !isAskQuestionTerminalState(state.phase);
  ...
  const selectOption = useCallback(
    (optionId: string) => {
      ...
      if (viewModel !== null) {
        ...
        setAskQuestionDraft(viewModel.questionId, viewModel.revision, {
          selectedOptionIds: selected,
    ...
    (value: string) => {
      ...
      if (viewModel !== null) {
        ...
        setAskQuestionDraft(viewModel.questionId, viewModel.revision, {
          ...
          freeText: maxLength === undefined ? value : value.slice(0, maxLength),
        });
      }
    },
    [viewModel],
  ...
  const beginSubmit = useCallback((): AskQuestionSubmitIntent | null => {
    const current = stateRef.current;
    ...
    if (
      ...
      isAskQuestionTerminalState(current.phase)
    ) {
      return null;
    }
  ...
  const resolveResult = useCallback((result: AskQuestionAnswerResult) => {
    dispatch({ type: 'result', result });
  }, []);

  const applyTranscriptStatus = useCallback(
    (status: Parameters<typeof transcriptStatusToUiState>[0]) => {
