...
<!-- Component content -->
<!-- Rich block frame -->
<!-- This surface: rich-block--frame — shared chrome for every rich card: header
     ...
     below. A designer may edit markup between the design-system slot seams below and
     nothing else. -->
<article class={`rich-block--frame${className.length > 0 ? ` ${className}` : ''}`}>
  <!-- This slot: header — eyebrow · title · metadata against status · redaction. -->
    ...
    <div class="rich-block--heading">
      ...
      {#if metadata.length > 0}
        <div class="rich-block--metadata">
          <!-- Do not edit — Metadata is derived data mapped to chips; edit the list only while it remains a bounded read-out. -->
  ...
  <!-- This slot: actions — footer row of Copy / Open unit actions. -->
  {#if actions !== undefined}<footer class="rich-block--actions">{@render actions()}</footer>{/if}
</article>

<style>
  /* Editable seam: layout — card geometry, padding, radius, shadow, frame width. */
  .rich-block--frame {
    min-inline-size: 0;
    inline-size: 100%;
  ...
  /* This slot: header — title/eyebrow/metadata versus status/redaction row. */
  /* Editable seam: layout — header gap, alignment, space-between rhythm. */
  .rich-block--header {
    display: flex;
    gap: var(--space-3);
  ...
  /* This slot: label — eyebrow kicker above the card title. */
  .rich-block--eyebrow {
  ...
  /* This slot: metadata · status · actions — shared flex row rhythm. */
  .rich-block--metadata,
  .rich-block--status,
  .rich-block--actions {
    display: flex;
  ...
  /* This slot: metadata — quiet factual chips (language, call, lines, labels). */
  .rich-block--metadata {
  ...
  /* This slot: metadata — inter-chip bullet separator. */
  .rich-block--metadata span + span::before {
    margin-inline-end: var(--space-2);
    content: '·';
  }

  /* This slot: status — lifecycle caption and redaction badge. */
  .rich-block--status {
    flex: 0 1 auto;
  ...
  /* This slot: content — the card body holding the per-card preview regions. */
  .rich-block--content {
    min-inline-size: 0;
  }

  /* This slot: actions — footer row of Copy/Open unit actions. */
  /* This state: open-handoff — the Open full screen action is a handoff into the
  ...
  /* Editable seam: layout — narrow reflow of the card at <=27rem: padding, header
