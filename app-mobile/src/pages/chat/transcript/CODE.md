...
- **`transcript-list.svelte`** — owns the `@tanstack/svelte-virtual` virtualizer. The virtualizer is a **store** (not a rune): reactive count is set via `$effect(() => $virtualizer.setOptions({ count, … }))` (setOptions merges, keeping injected observers), and elements measure via `{@attach}` + `data-index`.
- **`block.svelte`** — per-block layout; hands rich payloads to `../rich-content/rich-content-router.svelte`.
