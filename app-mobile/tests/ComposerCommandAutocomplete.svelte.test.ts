...
// escapeUnsafeName/optionId assertions are framework-agnostic pure functions
// and port verbatim against the Svelte-side module that exports them. The
// no-submit guarantee is structural: the derivation exposes only
// canInsert/canRetry — there is no submission field a state could
...
} from '../src/lib/chrome/ComposerCommandAutocomplete.svelte';
import CommandOption, { escapeUnsafeName, optionId } from '../src/lib/chrome/CommandOption.svelte';
...
describe('ComposerCommandAutocomplete rendering', () => {
  ...
  it('ready.filtered: only matching rows render with match emphasis', async () => {
    const items = [
  ...
  it('refreshing.current keeps rows visible with the checking message', async () => {
    renderPanel({
  ...
  it('ready.emptyCatalog and ready.noMatches render no listbox and no options', async () => {
  ...
  it('staleOffline keeps rows and shows the reconnect notice', async () => {
    renderPanel({
  ...
  it('session.running keeps rows insertable and explains the local boundary', async () => {
    renderPanel({
      running: true,
...
describe('CommandOption: safe text-only rows', () => {
  ...
  it('renders the canonical name isolated LTR, with hint, description, source, and confirmation', async () => {
    render(CommandOption, { props: optionProps() });
    await tick();
    const option = screen.getByRole('option');
  ...
  it('disabled rows replace the description with the disclosed reason and cannot activate', async () => {
    ...
    render(CommandOption, { props: { command, active: false, onInsert, onDisabledPress } });
    await tick();
    const option = screen.getByRole('option');
  ...
  it('enabled rows insert on completed press only', async () => {
    const onInsert = vi.fn();
    render(CommandOption, { props: { ...optionProps(enabledCommand, true), onInsert } });
    await tick();
    fireEvent.click(screen.getByRole('option'));
    expect(onInsert).toHaveBeenCalledWith('plan');
  });

  it('a tap-drag past the slop cancels activation even if a click completes', async () => {
    const onInsert = vi.fn();
    const { container } = render(
      CommandOption,
      { props: { ...optionProps(enabledCommand, false), onInsert } },
    );
    await tick();
  ...
  it('a drag within the slop still inserts (no accidental suppression)', async () => {
    const onInsert = vi.fn();
    const { container } = render(
      CommandOption,
      { props: { ...optionProps(enabledCommand, false), onInsert } },
  ...
  it('rows are not in the tab order and contain no interactive descendants', async () => {
    const { container } = render(CommandOption, { props: optionProps() });
    await tick();
    const option = container.querySelector('[role="option"]')!;
  ...
  it('renders an escaped display name without the bidi override in the DOM', async () => {
    const command: RankedHostCommand = {
      ...COMMANDS[0]!,
      name: 'plan\u202eevil',
      matchTier: 'host-order',
    ...
    const { container } = render(CommandOption, { props: optionProps(command) });
    await tick();
...
describe('applied panel styling stays inside the frozen system', () => {
  ...
  const css = [
    ...
    readFileSync('app-mobile/src/lib/chrome/ComposerCommandAutocomplete.svelte', 'utf8'),
    readFileSync('app-mobile/src/lib/chrome/CommandOption.svelte', 'utf8'),
  ].join('\n');
...
describe('useVisualViewportAnchor', () => {
  it('mirrors the fallback innerHeight into --visual-viewport-height after a frame', async () => {
    render(VisualViewportAnchorProbe, { props: { getAnchor: () => null } });
    await waitFor(() =>
