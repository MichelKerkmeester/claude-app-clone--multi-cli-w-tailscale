...
import { clearAskQuestionEphemeralStore } from '../src/features/ask-question/askQuestionEphemeralStore.js';
import type { DisplayTranscriptBlock, TranscriptState } from '../src/state.js';
...
function renderCard(
  ...
  return render(
    <AskQuestionCard block={currentBlock} principal="operator@example.test" />,
  );
}
...
describe('ask-question inline card', () => {
  ...
  it('keeps single and multiple selection local with no mutation on edit or blur', async () => {
    ...
    render(<AskQuestionCard block={block()} principal="operator@example.test" />);
    await waitForCard();
    await user.click(screen.getByRole('button', { name: OPTION_A }));
    await user.click(screen.getByRole('button', { name: OPTION_B }));
