...
describe('PdfPreview', () => {
  it('keeps page virtualization bounded and exposes verified text controls', async () => {
    ...
    // The Svelte input wires `oninput` (the DOM input event), where React's
    // onChange maps. fireEvent.input is the faithful equivalent here.
    await fireEvent.input(screen.getByRole('searchbox', { name: 'Search verified PDF text' }), {
      target: { value: 'safe' },
    });
