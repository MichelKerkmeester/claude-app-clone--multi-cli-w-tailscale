// ───────────────────────────────────────────────────────────────────
// MODULE: Plan Mode Button Tests
// ───────────────────────────────────────────────────────────────────
// Proves the full fail-closed presentation table: host-confirmed labels,
// consequence-bearing accessible names, the never-flash-unconfirmed-Build
// invariant, menu focus movement without mutation, and activation-only
// requests.
...
function renderButton(
  ...
  return { onSelectPlan, onSelectBuild, ...view };
}
...
describe('PlanModeButton DOM behavior', () => {
  ...
  it('activation alone reports the target; arrow focus movement never does', async () => {
    const user = userEvent.setup();
    const { onSelectPlan } = renderButton(readyWith(HOST_STATE));
    await user.click(screen.getByRole('button', { name: /Agent mode: Build/ }));
    const menu = await screen.findByRole('menu', { name: /Agent mode/ });
    expect(within(menu).getAllByRole('menuitem')).toHaveLength(2);
    // Arrow focus movement never mutates authority.
    await user.keyboard('{ArrowDown}');
    await user.keyboard('{ArrowDown}');
    await user.keyboard('{ArrowUp}');
    expect(onSelectPlan).not.toHaveBeenCalled();
    // Activation alone reports the target; the menu closes and focus
    // returns to the mode button.
    await user.keyboard('{Enter}');
    expect(onSelectPlan).toHaveBeenCalledTimes(1);
    expect(screen.queryByRole('menu')).not.toBeInTheDocument();
    await waitFor(() =>
      expect(screen.getByRole('button', { name: /Agent mode: Build/ })).toHaveFocus(),
    );
  });
