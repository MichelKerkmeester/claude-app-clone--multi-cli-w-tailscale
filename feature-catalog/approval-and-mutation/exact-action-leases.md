...
The card exposes the approval id, session, epoch, tool, redacted canonical arguments, digest, policy version, revision, requested and expiry times, source, and status. Leases move through pending, approved or denied, consumed, expired, revoked, and restart-invalidated states, and every transition writes an audit row.
