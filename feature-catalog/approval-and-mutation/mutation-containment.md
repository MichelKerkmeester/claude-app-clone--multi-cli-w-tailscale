...
`createFinalBoundaryHandler` builds the handler that `installPiRemoteApproval` registers on the Pi tool call event; it acts only for tools in the protected family set. For a protected call it builds the exact action with the configured principal, session, epoch, and policy version, digests it, requests a lease, and polls the consume route until the lease settles or its TTL passes.
