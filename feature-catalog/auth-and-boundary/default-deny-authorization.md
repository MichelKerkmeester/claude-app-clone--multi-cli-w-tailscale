...
Fixed-window limiters bound general requests and enrollment per principal and address, prompt submission per device, and never retain request payloads. A rejected bucket increments the rate-limited metric and returns a 429 response.
...
| File                                                            | Type        | Role                                                                 |
| --------------------------------------------------------------- | ----------- | -------------------------------------------------------------------- |
| `apps/pi-remote-relay/tests/security/negative-controls.test.ts` | Integration | Pins tool mutation denial as a fail-closed control |
