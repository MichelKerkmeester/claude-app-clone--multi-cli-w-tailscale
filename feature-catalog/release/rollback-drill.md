...
The damaged database is replaced from the backup, migrated down to version 6, and checked for the preserved session card and the indeterminate consumed lease. A sentinel file in the native session history is hashed before and after, and the drill fails if the restore, migration, or sentinel boundary changes. The report returns the drain count, migration version, and preserved counts.
