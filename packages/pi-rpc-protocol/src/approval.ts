...
/** Produce the only byte representation used to bind an approval to an action. */
export function canonicalizeApprovalAction(action: ApprovalAction): string {
...
export function sha256(value: string): string {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let index = 16; index < 64; index += 1) {
      ...
      const s0 = rotateRight(prior, 7) ^ rotateRight(prior, 18) ^ (prior >>> 3);
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const first =
        (h + upper + choice + (SHA256_CONSTANTS[index] ?? 0) + (words[index] ?? 0)) >>> 0;
