...
- [ ] True-390px light/dark CDP captures cover `ready`, `staged`, and `committing`. — PENDING: deferred to the feature-001 visual checkpoint (after phase `004`, the motion/visual-hardening phase, when the sheet is visually final). Requires a one-time `demo.ts` contract-sync to the Phase-1 expanded catalog + ticket endpoint. The 390px CDP harness is built and proven. Functional 390px rendering (one dialog, no nested overlay, listbox semantics, no horizontal overflow) is already proven by the DOM assertions below.
