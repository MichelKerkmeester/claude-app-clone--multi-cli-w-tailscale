...
|---|---|---|---|
| Parchment canvas | `--canvas: #f8f8f6`, `--ink: #24221f` | `--canvas: #181715`, `--ink: #f4f1eb` | Assistant Source Serif prose (`1.1875rem` / `line-height: 1.62`) |
...
|---|---|---|---|
| Default | `--ink` `#24221f` | `--ink` `#f4f1eb` | already tested |
...
- Overlay: `position: fixed; inset: 0; background: color-mix(in oklch, #24221f 40%, transparent)` light / `color-mix(in oklch, #000 55%, transparent)` dark. **No** `backdrop-filter` on the full viewport (blur ≤8px only if bounded; full-screen blur janks on A-series — motion-performance ref).
