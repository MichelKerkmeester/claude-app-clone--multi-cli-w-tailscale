...
Lens: **visual-motion** for an installable iPhone PWA (React 19 + Vite + Tailwind 4 + react-aria-components). Design system is locked: ink-on-parchment (bone `#f8f8f6` / carbon ink `#24221f` / clay `#d97757`; Inter + Source Serif 4), light + dark, WCAG AA. Inbound analog of F5; fullscreen **reuses F6**.
...
- **Theme-color mismatch (visual defect if F6 opens):** `App.tsx` sets `theme-color` to `#101319` / `#f4f5f7`, but canvas is `#181715` / `#f8f8f6`. The iOS status bar will flash a **different parchment** when the overlay paints. F6 must set `theme-color` to the overlay ink (`#24221f` light-room / `#181715` dark) on open and restore on close.
...
- Leading: close `Button`, 44×44, circular, `background: color-mix(in oklch, var(--ink-inverse) 12%, transparent)`, glyph `--ink-inverse`, 3:1 vs scrim (scrim is ~88% `#24221f` → check close glyph `#f8f8f6` — that pair is the existing `action-fg on action-bg` ≥ 4.5:1).
...
**Status bar:** on open, set `meta[name=theme-color]` to `#24221f` (light) / `#181715` (dark). Restore previous. Overlay `color-scheme: dark` so Safari form controls don’t flash white.
...
- Contrast: no `--ink-muted` text on the carbon scrim (too low). Overlay labels use `--ink-inverse` (`#f8f8f6` on `#24221f`).
