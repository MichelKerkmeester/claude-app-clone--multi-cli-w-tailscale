...
- **8 feature phases · 80 research iterations · 8 syntheses · 8 specs · 34 build phases** (the build-ready originals)
- **Plus 2 net-new build-ready features:** `009-ask-question` (3 phases) and `010-todos` (3 phases) — research-synthesized (single GPT-5.6 Luna pass, reference-screen-grounded), each with a feature spec + phase manifest + 3 build sub-phases. Packet total: **10 features · 40 build phases**. See `roadmap.md` → "Net-new features".
- Build order (see `roadmap.md`): **F1 → F2 → F3 → F4 → F6 → F7 → F5 → F8** = folder order `001 → … → 008`. The `F`-labels are the original grouping; folders are numbered by build order (F6=`005`, F7=`006`, F5=`007`, F8=`008`).
  (partials / UI-only first; the two security-crossing binary lanes — F5 upload=`007`, F8 inbound media=`008` — last, each gated on an adversarial security review)
...
│   │   ├── BRIEF.md              ← research question + target + output contract
│   │   ├── research.md           ← build-ready synthesized decision (canonical)
│   │   ├── reference-screens.md  ← real Mobbin/Refero captures + UI/UX direction
│   │   └── iterations/iteration-NNN.md  ← independent cited passes
