...
**Frozen — design system.** Ink-on-parchment: bone `#f8f8f6` / carbon ink `#24221f` / clay `#d97757`; Inter + Source Serif 4; light + dark; WCAG AA. Not changed by this packet.
...
**Structure.** One phase per feature, research-first. Each feature phase is a phase-parent whose research lives in `research/` (`research/research.md` is the build-ready decision, synthesized from cited passes in `research/iterations/`) with a lean `001-research/` phase child pointing at it; build sub-phases start at `002-`, one per phase in that feature's `implementation-phases.md`, each independently shippable and verifiable.
...
9. `009-ask-question` — inline question card for `pi`'s ask-question extension; answering is a one-use, revision-bound ticketed mutation (non-optimistic); new mutation surface, hard-gated on adversarial review.
...
**Open direction.**
- Reliability: a follow-on research loop gathers real reference screens per feature (Mobbin + Refero via code mode) — Dot, Perplexity, Claude, Manus, Grok, Pi, ChatGPT — so the bar rests on real references.
- All 10 features are research-complete and build-ready (40 build phases). Implementation runs autonomously via external models per `build-strategy.md` — Claude orchestrates and verifies; it never writes app code.
