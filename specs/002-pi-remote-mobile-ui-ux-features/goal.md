...
- `research/` + `001-research/` — the feature's research. `research/research.md` is the build-ready synthesized decision (from independent cited passes in `research/iterations/` plus `research/reference-screens.md`); `001-research/` is the lean spec-kit phase child that points at it. No build work starts until research is synthesized.
...
9. **`009-ask-question`** — inline question card for `pi`'s ask-question extension; answering is a one-use, revision-bound ticketed mutation (non-optimistic, host-confirmed). New answer-mutation surface; hard-gated on an adversarial security review. 3 build phases.
10. **`010-todos`** — read-only inline projection of `pi`'s todo/plan list (the phone never mutates it); grouped states, header-count progress, live updates. 3 build phases.
...
- **Autonomous build campaign.** All 10 features are research-complete and build-ready (40 build phases). Implementation runs autonomously via external models per [`build-strategy.md`](build-strategy.md) — Claude orchestrates and verifies; it never writes application code.
