...
> **Features 001 (`change-model`), 002 (`change-effort`), 003 (`slash-commands`), 004
> (`plan-mode-tab`), and 005 (`file-preview`) are BUILT and merged to `main`** (20 build phases;
> `main` at `41bef3e`). Features 006–010 pending. This packet lives in the app repo (`Mobile CLI`),
> tracked on `main`.
> **Resume:** read this file → `build-strategy.md` (the how + model roster) → the target
> feature's `research/research.md` + its build sub-phase docs, then build. Next: `006-rich-content-blocks`
> (Claude-style bash Command/Output cards + code/text artifact cards; **REUSES the `005` viewer shell** already built).
...
- **Features 006–010 — build-ready, not started.** Each has synthesized `research.md`, build-ready `spec.md` + `implementation-phases.md`, and numbered build sub-phases. **40 build phases total; 20 done.** Hard security gates before their Phase 1: `007-media-upload`, `008-inbound-media`, `009-ask-question` (adversarial review).
...
1. **Build feature `006-rich-content-blocks`** next (Claude-style bash Command/Output cards + code/text artifact cards with Copy + full-screen). It **REUSES the `005` `apps/pi-remote-web/src/artifacts/` viewer shell** (ArtifactCard, ArtifactViewerHost, TextPreview/CodePreview, useArtifactResource) already on `main` — so it should be mostly web-only, no new lane. Isolated worktree off `main`, dispatch each build phase to a working route (codex or cursor Luna Max — see §2), verify OUTSIDE any sandbox (typecheck + `npm test` + `npm run test:web` + 390px light/dark CDP), merge per feature. Then `007`–`010`.
