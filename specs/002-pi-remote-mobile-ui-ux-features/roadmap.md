...
> **Numbering key.** The `F`-labels are the original feature grouping; the on-disk folders are numbered by build order, so the two schemes diverge for the last four. Map: F1=`001-change-model`, F2=`002-change-effort`, F3=`003-slash-commands`, F4=`004-plan-mode-tab`, **F6=`005-file-preview`, F7=`006-rich-content-blocks`, F5=`007-media-upload`, F8=`008-inbound-media`**. The order above therefore equals folder order `001 → 002 → 003 → 004 → 005 → 006 → 007 → 008`. Two further net-new features — `009-ask-question`, `010-todos` — are pending research (see "Pending features" at the end).
...
3. **`009-ask-question` adversarial security/redaction review:** before `009` Phase 1 starts, review and sign off the new answer-mutation surface — the one-use, device/question/revision/digest-bound ticket; fail-closed on every binding failure; no double-submit (one `clientMutationId` → one commit); redaction of question/option/answer text at every boundary (transcript JSON, cache, push, logs, telemetry); host/extension plan-mode enforcement; and the prohibition on the phone minting authority or enabling `--full-access`. Although `009` reuses the shipped ticket path rather than adding a binary lane, the new host-affecting mutation is treated as a release-blocking gate.

F5, F8, and `009` build phases all remain gated on their security reviews above. Their `implementation-phases.md` manifests exist on disk (F5 = 5 phases, F8 = 6 phases, `009` = 3 phases), but no build phase is scheduled until the corresponding review signs off.
...
## Net-new features (build-ready)

Two further features are now research-synthesized and build-ready, each matching the 001–008 shape (research decision → feature spec → `implementation-phases.md` → 3 numbered build sub-phases):

- **`009-ask-question` — pi's ask-question extension.** Inline `AskQuestionCard` in the transcript; options + optional free-text; answering is a one-use, revision-bound **ticketed mutation** (non-optimistic, host-confirmed). 3 phases: protocol/authority/redaction → card + state machine → keyboard/a11y/visual/hardening.
- **`010-todos` — pi's todos surface.** Inline **read-only** `TodoProjectionBlock` (the phone never mutates the list — no checkbox/reorder/add/delete); grouped states, color-free ink glyphs, header count + hairline progress, live deltas. 3 phases: projection protocol/redaction → panel/grouping/glyphs → live deltas/a11y/hardening.

**Build-order placement:** both are independent of 001–008 and of each other; build them after the 8 originals. `009` reuses the shipped ticketed-mutation path; `010` adds no mutation lane. Neither reuses the F6 viewer. **Security gate (decided):** `009`'s new answer-mutation surface is hard-gated on an adversarial security/redaction review before Phase 1 (see Hard gates §3), on par with `007`/`008` — even though it reuses the existing ticket path rather than adding a binary lane. `010` is a read-only projection and adds no mutation gate.
