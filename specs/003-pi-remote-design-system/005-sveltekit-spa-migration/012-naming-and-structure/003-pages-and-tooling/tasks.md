...
- [ ] **T1.1** Confirm children 001 and 002 have landed and `shared/` is settled.
- [ ] **T1.2** Confirm the manifest covers every `pages/**` file, including stories and tests.
- [ ] **T1.3** Confirm 013 is not running concurrently.
- [ ] **T1.4** Record the current `@ds guardrail:` fence count and the CSS-corpus file count, so the
...
- [ ] **T2.1** `pages/chat/artifacts/` — 24 files, the largest feature folder.
- [ ] **T2.2** `pages/chat/chrome/` — 17 files; carries the sheet, menu and card prefixes.
- [ ] **T2.3** `pages/chat/rich-content/` — 12 files.
...
- [ ] **T2.8** Update the three `$shared` alias definitions if any sub-path is hard-coded.
- [ ] **T2.9** Update Storybook globs and regenerate the story ids.
- [ ] **T2.10** Regenerate the 009 coverage-gate allowlist rather than hand-editing it — after a
      hundred-file rename, hand-editing is transcription, and transcription is where a silent omission
      enters.
- [ ] **T2.11** Update the CSS-corpus builder's glob, then confirm the corpus is non-empty. The
      token-identity gate reads through it, so a stale glob would make the gate pass by reading nothing.
- [ ] **T2.12** Update both vitest web configs and any cwd-relative `readFileSync` path in a web test.

**Conventions stop-gap**

- [ ] **T2.13** Correct the naming section of the conventions authority — kebab-case, the closed prefix
...
- [ ] **T2.14** Land it through an isolated Public worktree. Never stage in the shared checkout, whose
...
- [ ] **T3.1** Nine program gates green from the final state, `validate.sh` invoked through its
      realpath.
...
- [ ] **T3.4** Token-identity 0/0/0 over a corpus confirmed to contain files.
- [ ] **T3.5** Catalog smoke green in both themes after the story ids shift.
- [ ] **T3.6** Build-output size and chunk manifest compared before and after, since a deeper folder
