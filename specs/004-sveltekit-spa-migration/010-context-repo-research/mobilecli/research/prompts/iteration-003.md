...
- Write iteration narrative to: specs/004-sveltekit-spa-migration/010-context-repo-research/mobilecli/research/iterations/iteration-003.md
- Write per-iteration delta file to: specs/004-sveltekit-spa-migration/010-context-repo-research/mobilecli/research/deltas/iter-003.jsonl
...
- **ALLOWED WRITE PATHS (the ONLY paths you may create, modify, or append to)**:
  - `specs/004-sveltekit-spa-migration/010-context-repo-research/mobilecli/research/iterations/iteration-003.md`, this iteration's narrative markdown
  ...
  - `specs/004-sveltekit-spa-migration/010-context-repo-research/mobilecli/research/deltas/iter-003.jsonl`, this iteration's delta JSONL
- **BANNED OPERATIONS (NEVER execute against any path)**: `rm`, `rm -rf`, `git rm`, `mv`, `sed -i` (including `sed -i ''`), `rmdir`, `find ... -delete`, shell output-redirect truncate `>` against any file not in the allowed-write list, and any tool call whose effect is to delete, rename, or replace a file outside the allowed-write list. Reading is unrestricted; **writing, renaming, and deleting are scoped**.
- **SCOPE VIOLATION PROTOCOL**: if your plan would require modifying any path NOT in the allowed-write list, you MUST STOP that action and emit a finding instead. Record the would-be mutation as a `scope_violation` entry in the iteration narrative (under a `## SCOPE VIOLATIONS` heading) and continue the research. NEVER execute the out-of-scope mutation. The research packet (`specs/004-sveltekit-spa-migration/010-context-repo-research/mobilecli/research/iterations/iteration-003.md` directory and parents) is the only zone for your writes; the researched target/topic surface is off-limits.
...
1. **Iteration narrative markdown** at `specs/004-sveltekit-spa-migration/010-context-repo-research/mobilecli/research/iterations/iteration-003.md` (path is pre-substituted for the current iteration number). Structure: headings for Focus, Actions Taken, Findings, Questions Answered, Questions Remaining, Next Focus.
...
3. **Per-iteration delta file** at `specs/004-sveltekit-spa-migration/010-context-repo-research/mobilecli/research/deltas/iter-003.jsonl` (path pre-substituted for the current iteration — e.g. `deltas/iter-001.jsonl`). This file holds the structured delta stream for this iteration: one `{"type":"iteration",...}` record (same content as the state-log append) plus per-event structured records (one per graphEvent, finding, invariant, observation, edge, ruled_out direction). Each record on its own JSON line. The reducer reads the combined state log + delta files to rebuild dashboards and registries after interruption or partial runs.
