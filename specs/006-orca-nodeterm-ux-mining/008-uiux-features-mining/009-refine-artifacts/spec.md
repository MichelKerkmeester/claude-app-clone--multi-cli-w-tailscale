---
title: "Phase 9 - Refine artifacts"
description: "Walk all 91 artifacts screenshots one at a time and treat each as a UI review: analyse what it shows, debug what is wrong, then optimise and refine the component. Fix the component rather than the story, re-capture, and prove each change with a before and after diff of the shot itself. Scope: 91 screenshots. Chain: after 008-screenshot-archive-integrity · before 010-refine-chrome."
trigger_phrases:
  - "refine artifacts spec requirements"
...
_memory:
  continuity:
    ...
    recent_action: "Authored the refine artifacts plan; no code yet."
...
| **Phase chain** | after `008-screenshot-archive-integrity` · before `010-refine-chrome` |
<!-- /ANCHOR:metadata -->
...
### Problem Statement
The artifacts surfaces cover artifact cards, previews, diffs, image and media surfaces, and 91 screenshots record how they actually render. Nothing has yet read those shots as a UI review. A green test suite says a component mounts and behaves; it says nothing about whether text overflows, controls crowd, two states look identical, hierarchy reads, spacing matches its siblings, or a touch target is large enough to hit. Those defects are invisible to every gate this repo runs and visible in one glance at the picture.
...
### In Scope
- Per-shot analysis of all 91 artifacts screenshots.
...
| Path | Action | Why |
|------|--------|-----|
| the `artifacts` component sources under `app-mobile/src` | Modify | targeted refinement driven by its own screenshot |
...
|----|-------------|---------------------|
| REQ-001 | Every one of the 91 shots in this group is reviewed individually and its verdict recorded. | No shot is skipped; each is marked either clean or carrying a named defect. |
...
1. All 91 shots reviewed with a recorded verdict.
...
**Depends on:** `008-screenshot-archive-integrity` and a trustworthy archive from `008-screenshot-archive-integrity`.
<!-- /ANCHOR:risks -->
...
Level 2. 91 shots to review; each fix is a targeted presentation change with an image diff as its proof.
<!-- /ANCHOR:complexity -->
