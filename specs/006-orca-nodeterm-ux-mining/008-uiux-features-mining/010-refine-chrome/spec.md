...
_memory:
  continuity:
    packet_pointer: "specs/006-orca-nodeterm-ux-mining/008-uiux-features-mining/010-refine-chrome"
    last_updated_at: "2026-08-28T20:00:00.000Z"
    last_updated_by: "claude-opus-5"
    recent_action: "Authored the refine chrome plan; no code yet."
...
| **Phase chain** | after `009-refine-artifacts` · before `011-refine-transcript` |
<!-- /ANCHOR:metadata -->
...
### Problem Statement
The chrome surfaces cover the composer, sheets, headers, palettes and runtime strip, and 50 screenshots record how they actually render. Nothing has yet read those shots as a UI review. A green test suite says a component mounts and behaves; it says nothing about whether text overflows, controls crowd, two states look identical, hierarchy reads, spacing matches its siblings, or a touch target is large enough to hit. Those defects are invisible to every gate this repo runs and visible in one glance at the picture.

### Purpose
Walk all 50 chrome screenshots one at a time and treat each as a UI review: analyse what it shows, debug what is wrong, then optimise and refine the component. Fix the component rather than the story, re-capture, and prove each change with a before and after diff of the shot itself.
<!-- /ANCHOR:problem -->
...
### In Scope
- Per-shot analysis of all 50 chrome screenshots.
...
| Path | Action | Why |
|------|--------|-----|
| the `chrome` component sources under `app-mobile/src` | Modify | targeted refinement driven by its own screenshot |
...
|----|-------------|---------------------|
| REQ-001 | Every one of the 50 shots in this group is reviewed individually and its verdict recorded. | No shot is skipped; each is marked either clean or carrying a named defect. |
...
1. All 50 shots reviewed with a recorded verdict.
...
**Depends on:** `009-refine-artifacts` and a trustworthy archive from `008-screenshot-archive-integrity`.
<!-- /ANCHOR:risks -->
...
Level 2. 50 shots to review; each fix is a targeted presentation change with an image diff as its proof.
<!-- /ANCHOR:complexity -->
