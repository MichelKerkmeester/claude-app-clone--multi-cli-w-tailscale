...
|-------|-----|---------------------|--------------|--------|--------|
| AC-001 | REQ-001 | Given the chat, When the model picker is opened and then closed, Then the transcript scrolls and a control accepts a click | Reproduce first: drive open→close in a browser, assert the failure, then assert after the fix that `scrollTop` changes on a wheel or programmatic scroll AND a button's click handler fires. `document.body` must carry no residual `overflow`, `position` or `pointer-events` override, and no element outside the sheet may retain `inert` or `aria-hidden` | Unmet | - |
