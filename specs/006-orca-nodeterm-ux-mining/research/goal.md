...
**Loop.** `opencode run --command deep/research` `:auto` `--spec-folder=specs/006-orca-nodeterm-ux-mining` `--max-iterations=20`, **no early convergence** (`--convergence-mode=off --stop-policy=max-iterations`), `NODE_PRESERVE_SYMLINKS=1` (symlink-containment fix). Findings → `specs/006-orca-nodeterm-ux-mining/research/`; full angles in `specs/006-orca-nodeterm-ux-mining/research-angles.md`.
