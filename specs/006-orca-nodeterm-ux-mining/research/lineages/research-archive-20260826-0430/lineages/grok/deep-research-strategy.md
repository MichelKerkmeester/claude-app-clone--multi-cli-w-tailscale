...
- Mutating `specs/006-orca-nodeterm-ux-mining/spec.md` or any path outside this lineage directory.
...
### Spec anchoring
`specs/006-orca-nodeterm-ux-mining/spec.md` is absent (`folder_state: no-spec`). This lineage's write surface is `specs/006-orca-nodeterm-ux-mining/research/lineages/grok` only, so the no-spec seed branch is skipped (would write outside the lineage). `research.md` in this directory remains canonical for the lineage.
...
- Source pointers (ours): `packages/pi-rpc-protocol/src/types.ts` `SessionCardDto` (`id`, `status`, `updatedAt`, `messageCount`); `app-mobile/src/pages/home/screen-home.svelte` (status pill, compacted id, `messageCount` blocks, coarse `relativeTime`, whole-card tap); `specs/006-orca-nodeterm-ux-mining/research-angles.md` (gaps + constraint).
...
- Write surface: `specs/006-orca-nodeterm-ux-mining/research/lineages/grok` only
