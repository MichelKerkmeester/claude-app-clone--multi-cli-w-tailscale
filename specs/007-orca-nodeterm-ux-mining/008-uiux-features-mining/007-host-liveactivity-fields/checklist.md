...
- [x] CHK-013 [P1] Every host-gated affordance is fail-closed inert with its field absent [evidence: every host-gated affordance renders nothing without its field, asserted per finding in the gated-surface suites]
<!-- /ANCHOR:code-quality -->
...
- [x] CHK-030 [P1] REQ-011 (SC-3): elapsed renders now; count segments absent with no counts, never faked; live against a fixture [evidence: the elapsed tick renders with no counts present, and each count segment appears only with its own fixture field]
...
- [x] CHK-034 [P1] a11y contract preserved on the touched surfaces [evidence: the live-activity row carries a status role and the subagent tail a 44px disclosure target, asserted in their suites]
<!-- /ANCHOR:testing -->
...
- [x] CHK-FIX-002 [P0] Same-class producer inventory: every surface reading the shared attention resolver audited (LA-1, phase-003 dock, Live Activity) [evidence: the three surfaces reading the shared resolver are the home card, the recent-sessions dock and now the live-activity arbitration]
...
- [x] CHK-FIX-005 [P1] Fixture matrix listed for each host-gated finding (absent-field and present-field rows) before completion is claimed [evidence: each host-gated finding has an absent-field and a present-field fixture row in its own suite]
<!-- /ANCHOR:fix-completeness -->
...
- [x] CHK-042 [P1] The LA local first-seen and latched-dismiss state stays client-only and never reaches the host [evidence: first-seen times and the dismissal latch live in device-local state only; no host call exists on either path]
<!-- /ANCHOR:security -->
...
- [x] CHK-051 [P1] Code comments carry durable WHY only (no spec/finding ids in code) [evidence: a hygiene scan over every changed file in `app-mobile/src` and `app-mobile/tests` returned no spec paths or finding ids]
<!-- /ANCHOR:docs -->
