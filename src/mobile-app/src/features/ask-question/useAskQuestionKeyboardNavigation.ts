...
export function useAskQuestionKeyboardNavigation(
  options: UseAskQuestionKeyboardNavigationOptions,
): RefObject<HTMLElement | null> {
  const cardRef = useRef<HTMLElement | null>(null);
  const previousFocusRef = useRef<HTMLElement | null>(null);
  const lastFocusedInCardRef = useRef<HTMLElement | null>(null);
  ...
  useEffect(() => {
    ...
    lastFocusedInCardRef.current = null;
  }, [options.identity]);
  ...
  useEffect(() => {
    ...
    options.errorId,
    options.errorVisible,
    options.enabled,
    options.freeTextRequired,
    options.hasFreeText,
    options.labelId,
    options.optionCount,
    options.optionsLabelId,
    options.statusId,
    options.terminal,
  ]);
  ...
  useEffect(() => {
    ...
    const onFocusIn = (event: FocusEvent): void => {
      ...
      if (option !== null && card.contains(option)) {
        lastFocusedInCardRef.current = option;
        setRovingOption(option);
      } else if (isAnswerStop(target)) {
        lastFocusedInCardRef.current = target;
      }
    ...
    const onKeyDown = (event: KeyboardEvent): void => {
      ...
      if (textControl !== null) {
        if (
          ...
          !(event.isComposing || compositionRef.current)
        ) {
          event.preventDefault();
          current.submit();
          return;
  ...
  useEffect(() => {
    const card = cardRef.current;
    if (card === null || enteredRef.current) return;
    const current = optionsRef.current;
  ...
  }, [options.enabled, options.hasFreeText, options.optionCount, options.terminal]);

  useEffect(() => {
    const card = cardRef.current;
    if (card === null || !options.terminal) return;
    const active = document.activeElement;
    if (
      lastFocusedInCardRef.current === null ||
      (active !== document.body && !card.contains(active))
    ) {
      return;
    }
    focusElement(card);
  }, [options.identity, options.terminal]);
